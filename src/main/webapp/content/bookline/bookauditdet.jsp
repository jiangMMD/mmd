        ...
        <form class="form-horizontal" id="validation-form-book" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-4 form-group">
                                <label class="control-label" for="form-book-product_price">签约价<span
                                        class="red">*</span></label>
                                <input class="form-control" id="form-book-product_price" name="product_price" value="${linebook.product_price}"
                                       placeholder="请输入商品价值"/>
                            </div>
            ...
            <div class="row" id="book_audit_info">
                <div class="col-xs-4 col-xs-offset-5">
                    <c:if test="${linebook.state == '8'}">
                        <button type="button" class="btn btn-success" onclick="completePay()">已确认打款（意外保障费）</button>
                    </c:if>

                    <c:if test="${linebook.state == '4'}">
                        <button type="button" class="btn btn-success" onclick="passAudit()">审核通过</button>
                        <button type="button" class="btn btn-error" onclick="unpassAudit()">审核拒绝</button>
                    </c:if>
                </div>
            </div>
        </form>
...
<script>
    ...
    function passAudit() {
        ...
        bootbox.confirm("确认审核通过该订单，审核通过之后请及时关注打款情况(微信端客户可能会手动打款)", function(res) {
            if(res) {
                myAjax("/linebook/passAudit", {bid: bid}, function(result){
                    if(result.code == '1') {
                        if(result.message) {
                            showInfo(result.message);
                        }else{
                            showSuccInfo("审核成功，请关注该客户意外保障费打款情况");
                        }
    ...
    function unpassAudit() {
        ...
        bootbox.dialog({
            title: "审核拒绝",
            message: "<input id='unpassaudit_memo'  class='form-control' placeholder='请输入该客户审核拒绝的原因？'/>",
            buttons: {
                ok: {
                    callback: function () {
                        ...
                        myAjax("/linebook/unPassAudit", {bid: bid, audit_unpass_reason: audit_cause}, function (result) {
                            if (result.code === 1) {
                                if(result.message) {
                                    showInfo(result.message);
                                }else{
    ...
    function completePay() {
        var bid = $("input[name=bid]").val();
        bootbox.confirm("请确保已收到350元意外保障服务费,在点击确认按钮", function(res) {
            if(res) {
                myAjax("/linebook/completePay", {bid: bid}, function(result){
                    if(result.code == '1') {
                        showSuccInfo("操作成功，请立即将产品发货给客户指定收货地点");
                        $("#book_audit_info").hide();
                    }else{
                        showErrorInfo(result.message);
                    }
