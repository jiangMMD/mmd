...
(function (factory) {
	...
	Tree.prototype = {
			...
			this.options.dataSource(treeData ? treeData : {}, function (items) {
				...
				$.each(items.data, function (index, value) {
					...
					if (value.type === 'folder') {
						...
						$entity = self.$element.find('[data-template=treeitem]:eq(0)').clone().removeClass('hide hidden').removeData('template').attr("data-id", value.id); // hide is deprecated
                        if(self.options.readOnly) {
                            $entity.addClass("tree-readonly");
                        }else{
                            $entity.removeClass("tree-readonly");
                        }
		...
		selectTreeNode: function selectItem(clickedElement, nodeType, canWrite) {
			if(this.options.readOnly && !canWrite) {
				return;
			}
		...
		selectFolder: function selectFolder(el, canWrite) {  //canWrite 只读情况下是否可以触发选中（程序触发）
			if (this.options.folderSelect) {
				this.selectTreeNode(el, 'folder', canWrite);
			}
		},

		changeReadState: function changeReadState() {
			//修改只读状态，取反
			this.options.readOnly = !this.options.readOnly;
			if(!this.options.readOnly) {
				this.allNode().removeClass("tree-readonly");
			}else{
                this.allNode().addClass("tree-readonly");
			}
		},

		changeCanWrite: function changeCanWrite() {
            this.options.readOnly = false;
            this.allNode().removeClass("tree-readonly");
		},

		changeReadOnly: function changeReadOnly() {
			this.options.readOnly = true;
			this.allNode().addClass("tree-readonly");
		},

		selectItem: function selectItem(el, canWrite) {
			if (this.options.itemSelect) {
				this.selectTreeNode(el, 'item', canWrite);
			}
		},
        disSelectAll: function disSelectAll() {
            var $self = this;
            var $sel = this.$element.find('.tree-selected');
            $sel.each(function(index, $this){
                $self.selectItem($this, true);
			})
		},
		mainNode: function mainNode() {
			return this.$element;
		},

		allNode: function selectedAllNode() {
            return this.$element.find('.tree-item');
		},
